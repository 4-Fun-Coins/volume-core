...
contract('VolumeJackpot', async (accounts) => {
    ...
    const testSettingWinners = async (milestone, nextMilestone) => {
        ...
        const expectedOrder = [user7, user6, user5, user4, user3, user2, user1];
        ...
        for (let index = 0; index < expectedOrder.length; index++) {
            const claimableAmount = await jackpot.getClaimableAmount(expectedOrder[index]);
            assert.equal(expectedOrder[index], winners[index]);
